...
function getUltimosTreinos() {
  return Object.keys(localStorage)
    ...
    .slice(0, 2)
    .map(k => JSON.parse(localStorage[k]).grupo);
}
...
function sugerirGrupo() {
  ...
  Object.keys(localStorage).filter(k => k.startsWith("treino_")).forEach(k => {
    ...
    score[t.grupo] += t.lista.reduce((s, ex) => s + (ex.peso || 2), 0) * fi * ft;
  });
  ...
  saida += `<p>Cooldown ativo: ${cooldown.join(", ") || "nenhum"}</p><ul>`;
...
window.onload = () => {
  carregarDados();
  const perfil = getPerfil();
  if (!perfil.nome) exibirCard("perfilCard");
  else exibirCard("mainCard");
};
